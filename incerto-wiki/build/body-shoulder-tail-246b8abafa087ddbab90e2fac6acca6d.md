...
print(np.round(normal_peak_shoulder_tail(), 3))
print(np.round(t_peak_shoulder_tail(df=3), 3))
```

The normal boundaries are approximately
`[-2.136, -0.662, 0.662, 2.136]`.  A Student-t distribution with three degrees
of freedom has wider tail boundaries but a similar inner shoulder boundary.
