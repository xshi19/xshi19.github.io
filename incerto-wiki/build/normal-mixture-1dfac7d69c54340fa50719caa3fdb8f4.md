...
print(np.round(mixture_pdf / normal_pdf, 3))
print(simple_normal_mixture.mean(a))
print(simple_normal_mixture.var(a))
print(3 * (1 + a**2))
```

The density ratio is not monotone near the center, but it grows rapidly in the
far tail.  The mean and variance remain `0` and `1`; the fourth moment rises to
`3 * (1 + a**2)`.
