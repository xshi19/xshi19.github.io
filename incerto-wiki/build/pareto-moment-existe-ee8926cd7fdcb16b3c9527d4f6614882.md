...
sample = pareto.rvs(b=alpha, scale=x_m, size=50_000, random_state=rng)
running_mean = np.cumsum(sample) / np.arange(1, sample.size + 1)

print(alpha / (alpha - 1))      # finite theoretical mean
print(running_mean[-5:])        # finite-sample path is still jumpy
print(np.mean(sample**2))       # variance does not exist for alpha <= 2
```
