...
print(round(float(mean), 4), round(float(variance), 4))
print(round(float(np.mean(sample)), 4), round(float(np.var(sample)), 4))
print(
    np.round(
        univariate_variance_gamma.pdf(
            np.array([-3.0, 0.0, 3.0]), alpha, beta, lam
        ),
        6,
    )
)
